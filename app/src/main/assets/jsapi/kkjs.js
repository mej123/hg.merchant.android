(function(window) {
	...
	*/
	...
	function readyOk() {
		...
		event.initCustomEvent("KuKuJSBridgeReady", true, true, null);
	        ...
	        case 'event':
	        {
	          if (typeof msgWrap[_EVENT_ID] === 'string' ) {
	        	  if(msgWrap[_EVENT_ID] == "sys:init"){
	        			if (window.KuKuJSBridge && window.KuKuJSBridge._hasInit) {
	        				console.log('hasInit, no need to init again');
	        			}else if(window.KuKuJSBridge){
	        				window.KuKuJSBridge._hasInit = true;
	        				readyOk();
	        			}
...
//        alert(window.myTimestamp + " _sendMessageQueue 是：" + messageQueueString);
	...
	if(!window.KuKuJSBridge){
		window.KuKuJSBridge = __KuKuJSBridge;
	}
