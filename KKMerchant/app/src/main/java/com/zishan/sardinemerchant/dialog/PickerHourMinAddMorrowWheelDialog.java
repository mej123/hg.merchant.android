...
public class PickerHourMinAddMorrowWheelDialog extends CommonDialog implements View.OnClickListener {
    ...
    private int mMStartHour;
    private int mMEndHour;


    public static PickerHourMinAddMorrowWheelDialog newInstance(String title) {
        PickerHourMinAddMorrowWheelDialog dialog = new PickerHourMinAddMorrowWheelDialog();
    ...
    public int getLayoutId() {
        return R.layout.dialog_hour_min_pick_add_morrow;
    }
        ...
        switch (v.getId()) {
            ...
            case R.id.dialog_confirm:
                if (mDialogListener != null) {
                    ...
                    //判断开始时间是否大于结束时间,次日则在原有时间上加24小时
                    if (mStartHour.contains("次日")) {
                        String startHourSubstring = mStartHour.substring(2, mStartHour.length());
                        mMStartHour = Integer.parseInt(startHourSubstring) + 24;
                    } else {
                        mMStartHour = Integer.parseInt(mStartHour);
                    }
                    if (mEndHour.contains("次日")) {
                        String endHourSubstring = mEndHour.substring(2, mEndHour.length());
                        mMEndHour = Integer.parseInt(endHourSubstring) + 24;
                    } else {
                        mMEndHour = Integer.parseInt(mEndHour);
                    }
