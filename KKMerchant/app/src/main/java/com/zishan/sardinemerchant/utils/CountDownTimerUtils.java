...
public class CountDownTimerUtils extends CountDownTimer {
    ...
    public void onTick(long millisUntilFinished) {//该方法在倒计时时调用
        ...
        mTextView.setBackgroundResource(R.drawable.custom_btn_bg_enabled_hui); //设置按钮为灰色，这时是不能点击的

        //如果想给按钮上的字设置颜色可以进行以下操作：
        SpannableString spannableString = new SpannableString(mTextView.getText().toString());  //获取按钮上的文字

        ForegroundColorSpan span = new ForegroundColorSpan(Color.GRAY);//设置文本颜色
        /**
         * public void setSpan(ObjectTest what, int start, int end, int flags) {
    ...
    public void onFinish() {//计时结束之后实现这个方法
        ...
        mTextView.setBackgroundResource(R.drawable.gradient_blue_shape_1);//custom_btn_bg_enabled_blue
    }
}
